...
class Report:
    ...
    def generate_actionable_summary(self):
        actionable_df = pd.read_csv(
            ...
            index_col=0)
        preprocessed_text = f"There are " \
                            f"{actionable_df['preprocessed'].eq(False).sum()}" \
                            f" batches that have not been pre-processed." \
                            f"\nAlso, preprocessing info is " \
                            f"missing for " \
                            f"{actionable_df['preprocessed'].isnull().sum()}" \
                            f" batches."

        processed_text = f"{actionable_df['processed'].eq(False).sum()} " \
                         f"batches have not been processed yet." \
                         f"\nAlso, processing info is missing for " \
                         f"{actionable_df['processed'].isnull().sum()} batches."

        az_vs_lts_text = f"Batch present in AZ but not in LTS:\n" \
                         f"semif-uploads: {actionable_df[(actionable_df['upload_azure']) & (actionable_df['upload_lts'] == False)].shape[0]} \n" \
                         f"semif-developed-images: {actionable_df[(actionable_df['developed_azure']) & (actionable_df['developed_lts'] == False)].shape[0]} \n" \
                         f"semif-cutouts: {actionable_df[(actionable_df['cutouts_azure']) & (actionable_df['cutouts_lts'] == False)].shape[0]}"  # nopep8
        return f"{preprocessed_text}\n{processed_text}\n{az_vs_lts_text}"
...
def main(cfg: DictConfig) -> None:
    ...
    actionable_summary = report.generate_actionable_summary()

    report.compose_slack_message(actionable_summary)
